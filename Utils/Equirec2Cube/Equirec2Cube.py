...
class Equirec2Cube:
    def __init__(self, batch_size, equ_h, equ_w, out_dim, FOV, RADIUS=128, CUDA=True):
        ...
        batch_size = 1
        R_lst = []
        #Thetas for projection in rads - left face, front face, right face and back face. CC at the center of the cube.
        theta_lst = np.array([-90, 0, 90, 180], np.float) / 180 * np.pi
        #Phis for projection in rads - Top face and bottom face. CC at the center of the cube.
        ...
        #Camera params
        self.out_dim = out_dim #Output dimension of the projection
        equ_cx = (equ_w - 1) / 2.0
        equ_cy = (equ_h - 1) / 2.0
        c_x = (out_dim - 1) / 2.0
        c_y = (out_dim - 1) / 2.0
        
        #Read paper for wangle and w_len
        wangle = (180 - FOV) / 2.0
        ...
        #Creating x,y,z-plane of projection
        ...
        #D is Euclidean distance between a point and 3 planes of projection
        D = np.sqrt(x_map**2 + y_map**2 + z_map**2)
        xyz = np.zeros([out_dim, out_dim, 3], np.float)
        
        #Converting the plane of projection into arrays for accessing
        ...
        for i, R in enumerate(R_lst):
            ...
            tmp_xyz = result.contiguous().view(1, out_dim, out_dim, 3) #contiguous doesnt change anythng
